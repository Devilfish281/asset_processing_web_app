// src/components/detail-body/BodyJobAIStep.tsx
"use client";
...
function BodyJobAIStep({ jobId, job }: BodyJobAIStepProps) {
  const statusLabel = job.status ?? "created";
  const statusText = statusLabel.replaceAll("_", " ");
  const assistantMessage =
    job.lastMsgContent ?? "Waiting for asset_processing_service response.";
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Review the latest assistant response for this job.
              </p>
            </div>
